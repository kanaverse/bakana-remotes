...
export class CollaboratorDBDataset {
     ...
     * @param {string|number} [options.rnaCountAssay=0] - See {@linkcode CollaboratorDBDataset#setCountAssayRnaName setCountAssayRnaName}.
     * @param {string|number} [options.adtCountAssay=0] - See {@linkcode CollaboratorDBDataset#setCountAssayAdtName setCountAssayAdtName}.
     * @param {string|number} [options.crisprCountAssay=0] - See {@linkcode CollaboratorDBDataset#setCountAssayCrisprName setCountAssayCrisprName}.
     * @param {?(string|number)} [options.rnaExperiment=""] - See {@linkcode CollaboratorDBDataset#setExperimentRnaName setExperimentRnaName}.
     * @param {?(string|number)} [options.adtExperiment="Antibody Capture"] - See {@linkcode CollaboratorDBDataset#setExperimentAdtName setExperimentAdtName}.
     * @param {?(string|number)} [options.crisprExperiment="CRISPR Guide Capture"] - See {@linkcode CollaboratorDBDataset#setExperimentCrisprName setExperimentCrisprName}.
     ...
     * If `null`, the first encountered assay is used.
     */
    setAdtCountAssay(name) {
        this.#adtCountAssay = name;
        return;
    }
     ...
     * If `null`, the first encountered assay is used.
     */
    setCrisprCountAssay(name) {
        this.#crisprCountAssay = name;
        return;
    }
     ...
     */
    setRnaExperiment(name) {
        this.#rnaExperiment = name;
        return;
    }
     ...
     */
    setAdtExperiment(name) {
        this.#adtExperiment = name;
        return;
    }
     ...
     */
    setCrisprExperiment(name) {
        this.#crisprExperiment = name;
        return;
    }
        ...
        if ("single_cell_experiment" in meta) {
            ...
            if ("alternative_experiments" in scmeta) {
                ...
                for (var i = 0; i < resolved.length; i++) {
                    ...
                    this.#alternative_order.push(resolved[i]);
                }
            }
        }
    }
    ...
    #count_loader(modality_name, experiment, assay, out_mat, out_rids, out_feats) {
        // Choosing the experiment of interest.
        let se_meta;
        let exp_name = experiment;
        if (experiment === null) {
            return;
        } else if (typeof experiment == "number") {
            if (experiment >= this.#alternative_order.length) {
                return;
            }
            se_meta = this.#alternative_order[experiment];
            exp_name = this.#metadata.single_cell_experiment.alternative_experiments[experiment].name;
        } else if (experiment === "") {
            se_meta = this.#metadata;
        } else {
            if (!(experiment in this.#alternatives)) {
                return;
            }
            se_meta = this.#alternatives[experiment];
        }
        ...
        if (typeof assay == "string") {
            ...
            if (!found) {
                throw new Error("cannot find assay '" + assay + "' for modality '" + modality_name + "'");
            }
        } else if (assay >= assmeta.length) {
            throw new Error("assay index " + String(assay) + " is out of range for modality '" + modality_name + "'");
        }

        // Fetching the matrix and loading it.
        let curass = assmeta[assay];
        let mat_id = adb.packId(this.#unpacked.project, curass.resource.path, this.#unpacked.version);

        return adb_utils.loadScranMatrix(baseUrl, mat_id, { getFun: this.constructor.#getFun, downloadFun: this.constructor.#downloadFun })
            .then(x => {
    ...
    async load({ cache = false } = {}) {
        ...
        tasks.push(this.#count_loader("RNA", this.#rnaExperiment, this.#rnaCountAssay, out_mat, out_rids, out_feats));
        tasks.push(this.#count_loader("ADT", this.#adtExperiment, this.#adtCountAssay, out_mat, out_rids, out_feats));
        tasks.push(this.#count_loader("CRISPR", this.#crisprExperiment, this.#crisprCountAssay, out_mat, out_rids, out_feats));

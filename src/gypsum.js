...
/**
 * Dataset derived from a SummarizedExperiment in the [**gypsum**](https://github.com/ArtifactDB/gypsum-worker) store.
...
export class GypsumDataset extends bt.AbstractDataset {
    ...
    static async create(project, asset, version, path, url = "https://data-gypsum.artifactdb.com") {
        ...
        const overall = url + "/" + project + "/" + asset + "/" + version;
        if (path !== null) {
            overall += "/" + path;
        }
        ...
        for (const k of Object.keys(man)) {
            const comp = k.split("/");
            let step = project + "/" + asset + "/" + version;
